...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = (df[df['salary'] == '>50K'].value_counts(subset=df['native-country']) * 100 / df.value_counts(subset=df['native-country'])).idxmax()
    highest_earning_country_percentage = round(df[(df['salary'] == '>50K') & (df['native-country'] == highest_earning_country)].shape[0]* 100 / df[df['native-country'] == highest_earning_country].shape[0], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')].value_counts(subset=df['occupation']).idxmax()
